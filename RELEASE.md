...
- `devhub-gpui-0.1.0-x86_64-pc-windows-msvc.zip` for Windows x64
- `devhub-gpui-0.1.0-x86_64-unknown-linux-gnu.tar.gz` for Linux x64
- `devhub-gpui-0.1.0-aarch64-apple-darwin.tar.gz` for macOS Apple Silicon
...
```powershell
Get-FileHash .\devhub-gpui-0.1.0-x86_64-pc-windows-msvc.zip -Algorithm SHA256
```

Linux or macOS:

```sh
sha256sum devhub-gpui-0.1.0-x86_64-unknown-linux-gnu.tar.gz
```
